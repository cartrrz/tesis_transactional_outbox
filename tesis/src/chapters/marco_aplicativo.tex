...
\input{diagrams/arquitectura_libreria}
...
La Figura~\ref{fig:ciclo_vida_evento} resume el ciclo de vida de un evento desde su registro inicial hasta su publicación exitosa o su marcado como fallido.

\input{diagrams/ciclo_vida_evento}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{} c c c @{}}
        \toprule
        \tableHeader{Reintento} & \tableHeader{Demora (s)} & \tableHeader{Tiempo acumulado} \\
        \midrule
        1 & 2 & 2s \\
        2 & 4 & 6s \\
        3 & 8 & 14s \\
...
La Figura~\ref{fig:secuencia_outbox_idempotencia} muestra la secuencia general de interacción entre el servicio productor, la tabla Outbox, el publicador, Kafka y el consumidor idempotente.

\input{diagrams/secuencia_outbox_idempotencia}
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.92\textwidth}{@{} l Y @{}}
        \toprule
        \tableHeader{Paquete} & \tableHeader{Responsabilidad} \\
        \midrule
