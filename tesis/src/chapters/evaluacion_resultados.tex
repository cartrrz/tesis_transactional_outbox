...
Donde $n$ es el total de eventos evaluados y $p$ es la probabilidad real de entrega exitosa. Con base en el resumen consolidado de resultados, se tiene $n = 43{,}100$ eventos y $x = 43{,}100$ entregas exitosas, por lo que la proporción muestral observada es:

\[
    \hat{p} = \frac{x}{n} = \frac{43{,}100}{43{,}100} = 1.0000
\]
...
\[
    P(X \geq 43{,}100 \mid n = 43{,}100, p_0 = 0.99) = 0.99^{43{,}100} = 7.53 \times 10^{-189}
\]
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{} Y c Y @{}}
        ...
        \midrule
        Eventos evaluados ($n$) & 43,100 & Tamaño de muestra consolidado \\
        Eventos entregados ($x$) & 43,100 & Todos los eventos fueron entregados \\
        ...
        Valor-p exacto & $7.53 \times 10^{-189}$ & Menor que $\alpha = 0.05$ \\
        Intervalo de confianza unilateral 95\% & $[99.993\%, 100\%]$ & El límite inferior supera el 99\% \\
        Límite superior de falla 95\% & $0.00695\%$ & Probabilidad máxima estimada de pérdida o falla de entrega \\
        \bottomrule
    \end{tabularx}
    \label{tab:modelo_estadistico_hipotesis}
\end{table}

Como el valor-p obtenido es mucho menor que el nivel de significancia $\alpha = 0.05$, se rechaza la hipótesis nula $H_0$ y se acepta la hipótesis alternativa $H_1$. Además, el límite inferior del intervalo de confianza unilateral al 95\% es 99.993\%, valor superior al umbral de 99\% establecido en la hipótesis. Por tanto, los datos experimentales proporcionan evidencia estadística suficiente para afirmar que la librería supera el nivel mínimo de entrega definido.

El mismo razonamiento se aplica a los eventos perdidos y a los duplicados procesados: al observarse cero casos en las pruebas, el límite superior estimado para una falla de entrega se mantiene por debajo de 0.00695\% con 95\% de confianza. Esto respalda estadísticamente que la pérdida y el procesamiento duplicado fueron reducidos de forma significativa dentro del entorno experimental evaluado.
